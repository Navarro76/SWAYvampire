vergit() {
    local archivo="/home/alex/.dotfiles/shell/_ver/git.txt"
    local menu="ESC:SALIR | 1:FLUJOS | 2:COMMITS | 3:SUBIR | 4:DIFF | 5:GITEA | 6:AVANZADO"
    ...
    # Lanzamiento
    eval "sed -n \"/# FLUJOS/,/#/p\" \"$archivo\" | head -n -1" | eval "$RENDER" | fzf \
        --height 80% \
        --reverse \
        --ansi \
        --header "Guía: $menu" \
        --prompt="Filtro: " \
        --bind "1:reload(sed -n '/# FLUJOS/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "2:reload(sed -n '/# COMMITS/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "3:reload(sed -n '/# SUBIR/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "4:reload(sed -n '/# DIFF/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "5:reload(sed -n '/# GITEA/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "6:reload(sed -n '/# AVANZADO/,/#/p' $archivo | head -n -1 | $RENDER)" \
