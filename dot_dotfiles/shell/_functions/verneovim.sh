verneovim() {
    local archivo="/home/alex/.dotfiles/shell/_ver/neovim.txt"
    local menu="MENÚ: 1:MODOS | 2:SALIR | 3:NAVEGACIÓN | 4:GRAMÁTICA | 5:PRO | 6:ARCHIVO | 7:VISUAL | 8:KEYMAPS | 9:PLUGINS"
    ...
    # Lanzamiento
    eval "sed -n \"/# MODOS/,/#/p\" \"$archivo\" | head -n -1" | eval "$RENDER" | fzf \
        --height 80% \
        --reverse \
        --ansi \
        --header "$HDR" \
        --prompt="Filtro: " \
        --bind "1:reload(sed -n '/# MODOS/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "2:reload(sed -n '/# SALIR/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "3:reload(sed -n '/# NAVEGACIÓN/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "4:reload(sed -n '/# GRAMÁTICA/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "5:reload(sed -n '/# PRO/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "6:reload(sed -n '/# ARCHIVO/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "7:reload(sed -n '/# VISUAL/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "8:reload(sed -n '/# KEYMAPS/,/#/p' $archivo | head -n -1 | $RENDER)" \
        --bind "9:reload(sed -n '/# PLUGINS/,/#/p' $archivo | head -n -1 | $RENDER)" \
